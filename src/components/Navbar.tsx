      ...
      >
        ...
        >
          ...
          <div className="relative z-10 w-full h-full bg-bg rounded-full flex items-center justify-center">
            <span className="font-display italic text-[13px] text-text-primary tracking-tighter">
              MS
            </span>
          </div>
        </button>
        ...
        <div className="flex items-center gap-1">
          {['Home', 'Work', 'Resume'].map((link) => {
            const lowerKey = link.toLowerCase();
            const isActive = activeTab === lowerKey;
            return (
              <button
                key={link}
                onClick={() => scrollToSection(lowerKey === 'home' ? 'hero' : lowerKey === 'work' ? 'works' : 'contact')}
        ...
        >
          ...
          <span className="relative z-10 inline-flex items-center gap-1 bg-surface px-3 sm:px-4 py-1.5 sm:py-2 rounded-full backdrop-blur-md text-text-primary group-hover:bg-bg transition-colors duration-200">
            Say hi <span className="text-xs transition-transform duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5">↗</span>
          </span>
        </a>
      </nav>
    </header>
